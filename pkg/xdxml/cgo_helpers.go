...
// Ref returns a reference to C object as it is.
func (x *Memory) Ref() *C.xdx_memory_t {
	if x == nil {
		return nil
	}
	return (*C.xdx_memory_t)(unsafe.Pointer(x))
}
...
// allocMemoryMemory allocates memory for type C.xdx_memory_t in C.
// The caller is responsible for freeing the this memory via C.free.
...
const sizeOfMemoryValue = unsafe.Sizeof([1]C.xdx_memory_t{})

// PassRef returns a reference to C object as it is or allocates a new C object of this type.
func (x *Memory) PassRef() *C.xdx_memory_t {
	if x == nil {
